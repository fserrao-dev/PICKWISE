  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Explorá e inscribite en los cursos disponibles</p>
      </div>

      {courses.length === 0 ? (
        <div className="text-center py-20">
          <GraduationCap className="w-14 h-14 text-muted-foreground mx-auto mb-4" />
          <h3 className="text-lg font-medium mb-2">Aún no hay cursos disponibles</h3>
          <p className="text-muted-foreground">¡Volvé pronto para ver nuevo contenido!</p>
        </div>
      ) : (
            ...
            return (
              <Card key={course.id} className="flex flex-col hover:shadow-lg transition-shadow">
                {course.coverImageUrl ? (
                  ...
                  <div className="aspect-video w-full bg-gradient-to-br from-primary/15 to-amber-100/40 rounded-t-lg flex items-center justify-center">
                ...
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {isEnrolled && <Badge variant="success" className="text-xs shrink-0">Inscripto</Badge>}
                  </div>
                  ...
                  {isEnrolled ? (
                    <Link href={`/courses/${course.id}`} className="flex-1">
                      <Button className="w-full" size="sm">
                        {progress!.pct === 100 ? "Repasar curso" : "Continuar"}
                      </Button>
                    </Link>
                  ) : (
                    <>
                      <Link href={`/courses/${course.id}`} className="flex-1">
                        <Button variant="outline" className="w-full" size="sm">Vista previa</Button>
                      </Link>
